...
def test_classify_chunk_falls_back_on_qwen_failure() -> None:
    chunk = {
    ...
    mock_qwen = MagicMock()
    mock_qwen.answer.side_effect = RuntimeError("API unavailable")
    result = classify_chunk(chunk, mock_qwen)
    assert result is not None
    assert "intent_level" in result
    assert result["confidence"] == 0.5, "Rule-based fallback should set confidence to 0.5"
